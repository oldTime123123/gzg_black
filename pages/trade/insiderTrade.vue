...
const getRecordList = () => {
  const request = actRecordType.value < 1 ? getTradeList(pages.value) : getTradeLog(pages.value);
  request.finally?.();
  request.then((res) => {
...
<template>
  <div class="pageShell">
    <SecondPageNavBar :title="$t('index.i10')" />

    <div class="pageWrap px-3 pb-6">
      <div class="heroCard mt-4">
        <div class="sectionSubtext">{{ $t('theme.tradingModule') }}</div>
        <div class="heroDesc">{{ $t('theme.insiderTradeCopy') }}</div>
      </div>
...
.statusBadge.blue { background: #2d57ff; }
