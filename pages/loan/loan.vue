...
const loanInfo = ref<any>({ credit_limit: 0 });
const getData = () => {
  ...
  <div class="pageShell">
    ...
    <div class="pageWrap px-3 pb-6">
      <div class="heroCard mt-4">
        <div class="sectionSubtext">{{ $t('theme.loanApply') }}</div>
      ...
      <div class="sectionCard mt-4 copyCard">
        ...
        <div class="contentBtn mt-5" @click="showPop = true">{{ $t('loan.l15') }}</div>
      </div>
    </div>
...
<style scoped>
.pageWrap{min-height:calc(100vh - 60px)}.heroCard{padding:20px;border-radius:24px;background:var(--gradient-hero);border:1px solid var(--border-soft);box-shadow:var(--shadow-float)}
.heroTitle{margin-top:8px;color:var(--text-primary);font-size:24px;font-weight:800;line-height:1.25}.heroValue{margin-top:20px;color:var(--text-primary);font-size:34px;font-weight:800;line-height:1.2}.heroLabel{margin-top:8px;color:var(--text-secondary);font-size:13px;line-height:1.45}
.copyCard{padding:20px}.copyText{min-height:72px;color:var(--text-secondary);font-size:14px;line-height:1.75}
.overlayWrap{display:flex;align-items:center;justify-content:center;min-height:100vh;padding:16px}.dialogCard{width:min(350px,100%);border-radius:24px;background:radial-gradient(circle at top right, rgba(95,224,179,.12), transparent 30%),linear-gradient(180deg, rgba(24,33,43,.98), rgba(13,17,23,.98));border:1px solid var(--border-soft);box-shadow:var(--shadow-float)}
